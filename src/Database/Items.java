...
public class Items {
    ...
    public void addNew() {
        try (Connection conn = DriverManager.getConnection(URL, HOST_NAME, PASSWORD);
                Statement stmt = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
                        query = "Insert into Store (itemID,itemQuantity,itemSize,itemDocPrice,itemCustomerPrice,buyPrice) Values ('"
                    + name + "',"
                    + quantity
                    + ",'" + size + "',"+docPrice+","+cusPrice+","+buyPrice+")";
    ...
    public void mafqood(int zz) {
        try (Connection conn = DriverManager.getConnection(URL, HOST_NAME, PASSWORD);
                Statement stmt = conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            query = "Insert into Mafqood (itemID,itemQuantity,itemSize) Values ('"
                    + name + "',"
                    + quantity
                    + ",'" + size + "')";
